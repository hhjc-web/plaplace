...
def solution(X):
    abs_coords = tf.abs(X)
    max_abs = tf.reduce_max(abs_coords, axis=1, keepdims = True)
    result = 1.0 - max_abs
    return result

def gradsolution(X):
    dist = 1 - tf.abs(X)
    min_dist = tf.reduce_min(dist, axis=1, keepdims=True)

    is_min = tf.abs(dist - min_dist) < 1e-12
    unique_min = tf.reduce_sum(tf.cast(is_min, tf.float64), axis=1) == 1

    sign_grad = -tf.sign(X)

    mask = tf.cast(is_min, tf.float64) * tf.cast(unique_min[:, tf.newaxis], tf.float64)

    ugrad = mask * sign_grad
...
def loss(GRAD, ROT, X_domain, X_bdy, u_bdy, domain_size, bdy_size):
    ...
    loss_pde = domain_size * tf.reduce_mean(1/q * tf.reduce_sum(sigma**2, axis = 1, keepdims = True)**(q/2))
                # + domain_size * tf.reduce_mean(tf.reduce_sum(sigma*ugrad, axis = 1, keepdims = True)))
...
sigma = -tf.reduce_sum(sigma**2, axis=1, keepdims = True)**((q-2)/2) * sigma
utest = sigma[:, 1:2]
utest = utest.numpy()
sio.savemat(os.path.join(output_path, "solution" + ".mat"),
            {"samples": Xtest.numpy(),
             "optimal_solution": solution(Xtest).numpy(),
             "pred_solution": utest})