...
def get_results(GRAD, X):
    ...
    # f = tf.where(r > 1, -2 * (r - 1) ** (2 * p - 3) * (p - 1 / r), 0)
...
def bdy_loss(GRAD, X_bdy):
    ...
    return tf.reduce_mean(tf.math.abs(u) ** q(X_bdy)) + tf.reduce_mean(tf.math.abs(utan) ** q(X_bdy))

def total_loss(GRAD, X_domain, X_bdy, lambda_bdy):
    _, f_residual = get_results(GRAD, X_domain)
    loss_pde = tf.reduce_mean(tf.math.abs(f_residual)**q(X_domain))
