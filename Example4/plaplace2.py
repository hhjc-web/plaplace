...
def p(x):
    return 1+((x[:,0:1]+x[:,1:2]+x[:,2:3])/3+3)**(-1)
...
def solution(X):
    dtype = X.dtype
    sqrt3 = tf.cast(tf.sqrt(3.0), dtype)
    e3 = tf.cast(tf.exp(3.0), dtype)
    result = e3 * sqrt3 * (tf.exp((X[:,0:1] + X[:,1:2] + X[:,2:3]) / 3) - 1)
    return result

def gradsolution(X):
    coefficient = 1 / tf.sqrt(tf.constant(3.0, dtype=X.dtype))
    grad_val = coefficient * tf.exp(3.0+(X[:,0:1] + X[:,1:2] + X[:,2:3])/3)
    ugrad = tf.concat([grad_val, grad_val, grad_val], axis=1)
    result = -tf.reduce_sum(ugrad ** 2, axis=1, keepdims=True) ** ((p(X) - 2) / 2) * ugrad
    return result
...
Xplot, x, y = generate_test(Nx, Ny)
Xplot = np.hstack((Xplot, 0*Xplot[:,0:1]))
Xplot = convert_to_tensor(Xplot)

Xtest = generate_inputs(100000)
...
def loss(GRAD, ROT, X_domain, X_bdy, u_bdy, domain_size, bdy_size):
    ...
    coefficient = 1 / tf.sqrt(tf.constant(3.0, dtype=X_domain.dtype))
    grad_val = coefficient * tf.exp(3.0 + (X_domain[:, 0:1] + X_domain[:, 1:2] + X_domain[:, 2:3]) / 3)
    ugrad = tf.concat([grad_val, grad_val, grad_val], axis=1)
...
with tf.GradientTape(persistent=True, watch_accessed_variables=False) as gt_u:
    gt_u.watch(Xplot)
    u = output(GRAD, Xplot)
ugrad = gt_u.gradient(u, Xplot)

with tf.GradientTape(persistent=True, watch_accessed_variables=False) as gt_v1:
    gt_v1.watch(Xplot)
    v1 = output(ROT, Xplot)[:, 0:1]
v1grad = gt_v1.gradient(v1, Xplot)

with tf.GradientTape(persistent=True, watch_accessed_variables=False) as gt_v2:
    gt_v2.watch(Xplot)
    v2 = output(ROT, Xplot)[:, 1:2]
v2grad = gt_v2.gradient(v2, Xplot)

with tf.GradientTape(persistent=True, watch_accessed_variables=False) as gt_v3:
    gt_v3.watch(Xplot)
    v3 = output(ROT, Xplot)[:, 2:3]
v3grad = gt_v3.gradient(v3, Xplot)
...
sio.savemat(os.path.join(output_path, "solution" + ".mat"),
            {"samples": Xplot.numpy(),
             "optimal_solution": gradsolution(Xplot).numpy(),
             "pred_solution": utest})