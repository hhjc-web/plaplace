...
# Boundary: \nabla u*n = h
    ...
    def Phi(
        ...
        grad = autograd.grad(ux, xclone, grad_outputs=torch.ones_like(ux), \
                             only_inputs=True, create_graph=True, retain_graph=True)[0]
        result = -torch.sum(grad ** 2, dim=1, keepdim=True) ** ((self.p - 2) / 2) * grad
        grad1 = autograd.grad(result, xclone, grad_outputs=torch.ones_like(result), \
                             only_inputs=True, create_graph=True, retain_graph=True)[0]
        plaplace = torch.sum(grad1, dim=1, keepdim=True)


        res_PDE = plaplace - self.f(x_domain)
        res_PDE = self.loss_function(res_PDE, 0*res_PDE)
