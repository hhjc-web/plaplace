...
class DeepNonsysElliptical(object):
    def __init__(
        ...
        self.solution_net_copy = clone_freeze(self.solution_net)
        # loss
    ...
    def _train_solution_one_step(
        ...
        loss = self.loss_fn.loss_step(self.solution_net, self.solution_net_copy, first, data_domain, data_boundary,data_interface)
        # if(first):
    ...
    def _compute_error(self, error_samples: Tensor) -> Tuple[float]:
        ...
        x_clone = error_samples.clone().detach().requires_grad_(True)
        pred = self.solution_net(x_clone)
        grad = autograd.grad(pred, x_clone, grad_outputs=torch.ones_like(
            pred), create_graph=True, only_inputs=True)[0]
        pred_grad_solution_value = -torch.sum(grad ** 2, dim=1, keepdim=True) ** ((self.p - 2) / 2) * grad
    ...
    def _save_results(self, plot_samples: Tensor) -> None:
        r = torch.sum(plot_samples ** 2, dim=1, keepdim=True)
        pred_solution_value = self.solution_net(plot_samples)
        pred_solution_value = pred_solution_value.cpu()
        x_clone = plot_samples.clone().detach().requires_grad_(True)
        pred = self.solution_net(x_clone)
        grad = autograd.grad(pred, x_clone, grad_outputs=torch.ones_like(
            pred), create_graph=True, only_inputs=True)[0]
        pred_grad_solution_value = -torch.sum(grad ** 2, dim=1, keepdim=True) ** ((self.p - 2) / 2) * grad
