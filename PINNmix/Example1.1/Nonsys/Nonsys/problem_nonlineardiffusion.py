...
# Boundary: \nabla u*n = h
    ...
    def Phi(
        ...
        grad = autograd.grad(ux, xclone, grad_outputs=torch.ones_like(ux), \
                             only_inputs=True, create_graph=True, retain_graph=True)[0]
        result = -torch.sum(grad ** 2, dim=1, keepdim=True) ** ((self.p - 2) / 2) * grad
        divp = autograd.grad(p(xclone)[:, 0:1], xclone, grad_outputs=torch.ones_like(p(xclone)[:, 0:1]), \
                             only_inputs=True, create_graph=True, retain_graph=True)[0][:, 0:1]
        for i in range(1,self.ndim):
            divp = divp + autograd.grad(p(xclone)[:, i:i+1], xclone, grad_outputs=torch.ones_like(p(xclone)[:, i:i+1]), \
                             only_inputs=True, create_graph=True, retain_graph=True)[0][:, i:i+1]

        res_PDE = self.loss_function(p(x_domain), result) + self.loss_function(divp, -self.f(x_domain))
